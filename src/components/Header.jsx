...
import { LOGO_URL, BRAND } from "../firebase/branding";

export default function Header({ user, customerId, onLogout }){
  return (
      ...
      <div className="container header-inner">
        <Link to="/" className="brand">
          <img
            ...
            onError={e => { e.target.onerror = null; e.target.src = "/src/assets/react.svg"; }}
          />
        ...
        <nav className="nav">
          {user ? (
            <div className="auth-info">
              {customerId && <span className="badge">Customer ID: <strong>{customerId}</strong></span>}
              <button className="primary-btn" onClick={onLogout}>Logout</button>
            </div>
          ) : (
            <div className="auth-links">
              <Link className="primary-btn" to="/auth">Login / Create Account</Link>
              <Link className="admin-btn" to="/admin">Admin Login</Link>
            </div>
          )}
        </nav>
      </div>
